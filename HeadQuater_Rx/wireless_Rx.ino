...
const byte address[6] = "00001";  // RF24 address
    ...
    if (receivedString.startsWith("200,")) {  
      ...
      for (int i = 0; i < sizeof(userData) / sizeof(userData[0]); i++) {
        if (receivedID == userData[i].id) {
          ...
          if (!idAlreadyStored && dataCount < 20) {
...
void handleRoot() {
  ...
  page += "}); }, 2000);";  // Check for new data every 5 seconds
  page += "</script>";
  page += "<style>";
  ...
  page += "<div class='container'><h1>Accident Monitoring System</h1>";
...
void handleAccept() {
  ...
  radio.stopListening();  // Switch to transmitter mode
      radio.openWritingPipe(address);  // Send back to 00001
